...
class CIFAR10:
    def __init__(self, batch_size=100, validation_proportion=0.1, augment_data=False):
        ...
        if not data_available:
            ...
            path_to_extract = os.path.abspath(os.path.join(DIR_BINARIES, '../'))
        ...
        # Normalize data
        mean = self.train_data.mean(axis=(0, 1, 2), keepdims=True)
        std = self.train_data.std(axis=(0, 1, 2), keepdims=True)
...
if __name__=='__main__':
    cifar10 = CIFAR10(batch_size=64, validation_proportion=0.1, augment_data=False)
